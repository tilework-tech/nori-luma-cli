...
- **`helpers.ts`** is the test infrastructure backbone, providing:
  - `createMockLumaService()` -- Returns a `LumaService` implementation backed by in-memory Maps and arrays for direct state manipulation in test setup. Maps are used for entities that need key-based lookup (e.g., `.events`, `.guests`, `.members`), while arrays are used for collection-style entities (e.g., `.calendarAdmins`, `.orgCalendars`, `.orgEvents`). Also exposes a `.calendar` object for modifying the calendar returned by `getCalendar`
  ...
  - Factory functions (e.g., `makeEvent()`, `makeGuest()`, `makeOrgEvent()`) -- Create domain objects with sensible defaults and partial override support; one factory per domain type
...
- The mock service's calendar and organization operations store state in arrays rather than Maps because these entities are singletons or small collections. The mock tracks `last*Params` properties (e.g., `lastTransferEventParams`, `lastCreateCalendarParams`, `lastListOrgEventsParams`) so tests can assert on parameters passed to void-returning or hard-to-observe methods
