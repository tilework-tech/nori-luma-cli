...
- Test structure mirrors source structure: `@/tests/program.test.ts` tests the root program, `@/tests/commands/` has a test file per command group (events, guests, hosts, ticket-types)

### Core Implementation

- **`helpers.ts`** is the test infrastructure backbone, providing:
  - `createMockLumaService()` -- Returns a `LumaService` implementation backed by in-memory Maps (`.events`, `.cancellationTokens`, `.guests`, `.hosts`, `.ticketTypes`) for direct state manipulation in test setup
  ...
  - `makeEvent()`, `makeGuest()`, and `makeTicketType()` -- Factory functions for `LumaEvent`, `LumaGuest`, and `LumaTicketType` objects with sensible defaults and partial override support
