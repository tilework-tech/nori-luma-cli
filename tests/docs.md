...
- **`helpers.ts`** is the test infrastructure backbone, providing:
  ...
  - Factory functions (`makeEvent()`, `makeGuest()`, `makeTicketType()`, `makeCalendar()`, `makeAdmin()`, `makeCoupon()`, `makeEventTag()`, `makeContact()`, `makeContactTag()`) -- Create domain objects with sensible defaults and partial override support
