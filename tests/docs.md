...
- The mock service's calendar, organization, and webhook operations store state in arrays rather than Maps because these entities are singletons or small collections. The mock tracks `last*Params` properties (e.g., `lastListEventsParams`, `lastCreateEventParams`, `lastUpdateEventParams`, `lastAddEventParams`, `lastTransferEventParams`, `lastCreateCalendarParams`, `lastCreateWebhookParams`, `lastCreateUploadUrlParams`) so tests can assert on parameters passed to methods -- especially useful for verifying complex nested parameters (JSON-parsed objects, coordinate pairs) are correctly forwarded from CLI flags to service calls
