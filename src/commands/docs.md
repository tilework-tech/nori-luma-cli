...
- Command groups cover all Luma domain entities and utility endpoints: events (CRUD, cancel, and event-scoped coupons), guests (list, get, add, update status, send invites), hosts (add, update, remove), ticket-types (CRUD + delete), calendar (settings, admins, calendar-scoped coupons, event tags, event submissions), contacts (list, import, contact tags CRUD, tag apply/unapply), membership (list tiers, add member, update member status), organization (org admins, org calendars, org events, transfer events, create calendars), webhook (list, get, create, update, delete), and utility (get-self, entity-lookup, image-upload)
...
- Both `calendar create-coupon` and `events create-coupon` discriminate on `--discount-type`: `percent` sends `percent_off`, while `amount` sends `cents_off` and `currency`. Calendar coupons apply across all events in a calendar (scoped by API key), while event coupons are scoped to a single event via `--event-id`. The `events update-coupon` command returns void from the API so outputs synthetic confirmation JSON (`{ updated: true, code }`)
