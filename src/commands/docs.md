...
- Command groups cover the main Luma domain entities: events (CRUD + cancel), guests (list, get, add, update status, send invites), hosts (add, update, remove), and ticket-types (CRUD + delete)

### How it fits into the larger codebase

- `@/src/program.ts` calls each command factory (e.g., `createEventsCommand`, `createGuestsCommand`, `createHostsCommand`, `createTicketTypesCommand`) and attaches the result to the root Commander program via `addCommand`
...
- Read operations (e.g., `events list`, `guests get`) output the full API response as JSON. Write-only operations where the API returns empty `{}` (e.g., `guests add`, `hosts add`, `hosts remove`, `ticket-types delete`) output confirmation JSON like `{ added: true, event_id }` or `{ deleted: true }` instead
