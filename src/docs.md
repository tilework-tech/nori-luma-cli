...
- **`program.ts`** -- Builds the Commander `Command` tree. Registers all command groups (events, guests, hosts, ticket-types, calendar). Recursively applies `configureCommandOutput` to redirect all Commander output through the `Output` interface
