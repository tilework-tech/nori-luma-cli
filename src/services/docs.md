...
| `listWebhooks`         | GET         | `/v1/webhooks/list`                 |
| `getWebhook`           | GET         | `/v2/webhooks/get`                  |
...
- GET endpoints (`listEvents`, `listGuests`, `getEvent`, `getGuest`) map camelCase params to the API's snake_case query params; POST endpoints pass params directly as JSON body. `CreateEventParams` and `UpdateEventParams` include complex nested fields (`coordinate`, `registration_questions`, `feedback_email`, `geo_address_json`) that are passed through as structured JSON in the POST body
...
- `addEventToCalendar` accepts a discriminated union param type (`AddEventLumaParams | AddEventExternalParams`) keyed on `platform: "luma" | "external"` -- the two shapes have completely different fields. The `external` variant includes optional geo fields (`geo_address_json`, `geo_latitude`, `geo_longitude`, `coordinate`) for location data
...
- The `listEvents` and `listContacts` methods do not use the shared `request` helper -- they manually construct the fetch call because they need `url.searchParams.append` (not `set`) for array parameters (`platforms` for listEvents, `tags` for listContacts), which require repeated query params (e.g., `?platforms=luma&platforms=external`). Both methods duplicate the auth header and error handling logic from the `request` helper. Contact tag endpoints use `/v1/calendars/` (plural) consistently, unlike the split seen in event-related endpoints
